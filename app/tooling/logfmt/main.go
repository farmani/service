// This program takes the structured logs output and makes it readable.
package main
...
func main() {
	...
	for scanner.Scan() {
		...
		// Build out the know portions of the logs in the order
		// I want them in.
		b.Reset()
		...
		for k, v := range m {
			switch k {
			case "service", "time", "caller", "level", "trace_id", "message":
				continue
			}
		...
		// Write the new logs format, removing the last :
		out := b.String()
